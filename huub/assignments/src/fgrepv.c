...
#define debug(l, format, ...) DEBUG( l, "", format __VA_OPT__(,) __VA_ARGS__)

#define USAGE(name)	fprintf(stderr, "Usage: %s [-cilnqsq][-D LEVEL] PATTERN [file ...]\n", name)
...
#define OPT_INVERT_MATCH	(uint8_t)64
#define OPT_NO_ERR_PRINT	(uint8_t)128
	...
	while ( inFlags && argv[*argIndex] && argv[*argIndex][0] == '-' ) {
		...
		while ( inFlags && argv[*argIndex][c] ) {
			...
			case '-':
				...
				break;
			case 'D':
				if ( argv[*argIndex][c+1] ) { 
					DEBUG_LEVEL(atoi(argv[*argIndex]+c+1));
					c++;
					break;
				}
				*argIndex +=1;
				if ( argv[*argIndex] ) { 
					DEBUG_LEVEL(atoi(argv[*argIndex]));
					skipToNextArg = true;
					break;
				}
				USAGE(argv[0]);
...
bool	grepInFile( char * pattern, FILE * f, char * fName, uint8_t options) {
	...
	size_t	lineNr = 0;
	size_t	nMatches = 0;
	bool	inverter = (options & OPT_INVERT_MATCH ? true : false);
	debug(1, "grepInFile(file='%s',pattern='%s')\n", fName, pattern);
	while (true) {
		lineNr++;
		r = getline(&line, &bufSize, f);
		debug(3, "grepInFile(file='%s'): lineNr=%zu getline() r=%zd bufSize=%zu\n", fName, lineNr, r, bufSize);
		if ( r == -1 ) break; // EOF
		if ( line[r-1] == '\n' ) line[r-1] = '\0'; // strip newline
		
		if ( inverter ^ lineMatches(line, pattern, options) ) {
			debug(2, "grepInFile(file='%s',pattern='%s') lineNr=%zu MATCH_TRUE line='%s'\n", fName, pattern, lineNr, line);
			nMatches++;
			...
			debug(2, "grepInFile(file='%s',pattern='%s') lineNr=%zu MATCH_FALSE line='%s'\n", fName, pattern, lineNr, line);
		}
	}
...
bool lineMatches( char * line, char * pattern, uint8_t options) {
	bool	match = false;
	char *	initialLine = line;
	debug(3, "lineMatches(line='%s',pattern='%s') :: %s\n", initialLine, pattern, "SEARCHING");
	if ( ! *pattern ) match = true;
	while ( *line && ! match ) {
		match = sStartsWithP(line,pattern,options);
		line++;
	}
	debug(3, "lineMatches(line='%s',pattern='%s') :: %s\n", initialLine, pattern, (match ? "MATCH_TRUE" : "MATCH_FALSE"));
	return match;
}
...
inline bool	sStartsWithP( char * s, char * p, uint8_t options) {
	...
	debug(4, "sStartsWithP('%s','%s')\n", s, p);
	while (*s && *p ) {
		if ( options & OPT_IGNORE_CASE ) {
			debug(5, "sStartsWithP: lower(*s),lower(*p) = %c %c\n", LOWER(*s), LOWER(*p));
			if ( LOWER(*s) != LOWER(*p) ) return false;
		} else {
			debug(5, "sStartsWithP: *s,*p = %c %c\n", *s, *p);
			if ( *s != *p) return false;
		}
		s++;
		p++;
