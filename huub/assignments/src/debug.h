...
#define DEBUG_TAG(tags)		debugState_(4, (int8_t)0, tags)
#define DEBUG_UNTAG(tags)	debugState_(8, (int8_t)0, tags)
#define DEBUG_ERROR		(uint8_t)128

uint8_t	debugState_( uint8_t action, int8_t level, char * tags) {
	//current implementation only supports checking level. no tags yet
	//static char *	debugTags[] = {};
	static int8_t	debugLevel = 0;
	uint8_t		returnLevel;
